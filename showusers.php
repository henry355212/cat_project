...
<html>
  ...
  <noscript>
    ...
    <script type="text/javascript">
      function DeletePhoto(userss_id, photo_id)
      {
        if (confirm("請確認是否刪除此相片？"))
          location.href = "delPhoto.php?userss_id=" + userss_id + "&photo_id=" + photo_id;
      }
    </script>
  </head>	
  <body>
    ...
    <?php 
      ...
      echo "<p align='center'>$userss_name</p>";
													
      //取得相簿裡所有照片的縮圖
      $sql = "SELECT id, name, filename FROM photo WHERE userss_id = $userss_id";
      $result = execute_sql($link, "userss", $sql);
      ...
      while ($row = mysqli_fetch_assoc($result))
      {
        ...
        echo "<td width='160px'><a href='photoDetail.php?userss=$userss_id&photo=$photo_id'>
        ...
        if ($userss_owner == $login_users)
          echo "<br><a href='editPhoto.php?photo_id=$photo_id'>編輯</a> 
               <a href='#' onclick='DeletePhoto($userss_id, $photo_id)'>刪除</a>";
      ...
      if ($userss_owner == $login_users)
        echo "<a href='uploadPhoto.php?userss_id=$userss_id'>上傳相片</a> ";
    ?>
